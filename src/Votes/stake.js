...
const Stake = (props) => {
  ...
  const checkTheBalance = async (e) => {
    ...
    //let account = await web3js.eth.getAccounts()
    let result = await contractPHNX.methods.balanceOf(accounts).call();
  ...
  useEffect(() => {
    ...
    calculateSpotInterest();
    getRatio();
    if (!durationDaysInput || !amountPhnxInput) {
      setZeroAmountEntered(false);
    }
  }, []);
  const temp = async () => {
  ...
  const checkWeb3 = async () => {
    ...
    } else if (temp.network != ethereumNetwork) {
      setEthereumNetworkError(true);
      setConnectMetaMask(false);
    } else if (temp.address !== props.address) {
  ...
  const submitProposalOnBlockchain = async () => {
    ...
    const onSubmit = await (
      await ContractInit.initPhnxStakingContract()
    ).methods
      .stakeALT(web3js.utils.toWei(amountPhnxInput), durationDaysInput)
  ...
  const addStake = async (input) => {
    if (!durationDaysInput || !amountPhnxInput) {
      ...
      const stakeOnProposal = await axios.post(
        ...
        <Grid container item justify="space-between">
          ...
          <Grid item>
            <Button
              ...
              disabled={checkVoted() || approvalLoader || myLoading}
              variant="outlined"
              style={{
        ...
        >
          <Grid style={{ position: "relative" }} xs={12} sm={3} item>
            ...
            <input
              ...
              disabled={
                checkVoted() || approvalLoader || sameAccountError == true
                  ? true
                  : !props.stakeLoading
                  ? myLoading
                    ? true
          ...
          <Grid xs={12} sm={1} style={{ display: "flex" }} item></Grid>
          <Grid style={{ position: "relative" }} xs={12} sm={3} item>
            <span style={{ color: "#828282", marginLeft: "16px" }}>
              Duration (days)
            </span>
          ...
          {!checkVoted() && (
            <Button
              ...
              disabled={
                ...
                ethereumNetworkError == true ||
                sameAccountError == true ||
                amountEntered == true ||
                zeroAmountEntered == true ||
                !durationDaysInput ||
                !amountPhnxInput
                  ? true
                  : !props.stakeLoading
