...
import { URL, NumioAuth } from "../../../const";
...
const LogInWithNumio = (props: any) => {
  ...
  const temp = async () => {
    const numioCDN = new numio({
      // apiURL: "https://api-pay-dev.numio.one/",
      app_id: "8464920",
    });
    const res: any = await numioCDN.getUrlForLogin();
    console.log("res -->", res);
    ...
    const callRes = await numioCDN.call(async (token: any) => {
      ...
      let body = {
        token: token.token,
        app_secret: "e6f0a1fbb4",
        userDetails: [token.first_name, token.email],
      };
